...
export function CircleSignalsPanel({ circleId, circleTitle }: Props) {
  ...
  async function submit(signalType: CircleSignalType, note?: string) {
    ...
    if (!userId) {
      setFeedback(
        "Para dejar una señal necesitamos tu identidad en este dispositivo. Retomá o creá tu perfil.",
      );
      return;
    }
  ...
  return (
    <section id="circle-signals" className="w-full max-w-md scroll-mt-4 rounded-2xl border border-[#E8EEF3] bg-white p-4">
      <p className="text-[13px] leading-relaxed text-[#6B7A8C]">
        ...
        ámbitos empiezan a reunir interés real. No abre membresía automática.
      </p>

      <div className="mt-3">
        <CommunityActionGate
          returnTo={returnTo}
          gateHint="Dejá una señal con perfil en este dispositivo. Si ya lo creaste, retomaló."
        >
                    ...
                    {ideaOpen ? (
                      <div className="mt-2">
                        ...
                        <button
                          ...
                          onClick={() => void submit(action.type, ideaText)}
