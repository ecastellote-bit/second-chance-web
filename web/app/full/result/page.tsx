    ...
    <main className="min-h-screen bg-white text-black px-6 py-10">
      <div className="max-w-4xl mx-auto space-y-8">
        <div className="space-y-2">
          ...
          <p className="text-base text-neutral-700">{reading.summaryForUser.diagnostico}</p>
        </div>

        <div className="grid gap-5">
          <section className="border rounded-xl p-5 space-y-2">
            <h2 className="text-lg font-medium">Tensión dominante</h2>
            <p className="text-sm text-neutral-700">{reading.dominantTension}</p>
          </section>

          <section className="border rounded-xl p-5 space-y-2">
            <h2 className="text-lg font-medium">Hilo conductor</h2>
            <p className="text-sm text-neutral-700">{reading.summaryForUser.hilo_conductor}</p>
          </section>

          <section className="border rounded-xl p-5 space-y-2">
            <h2 className="text-lg font-medium">Direcciones plausibles</h2>
              ...
              {reading.plausibleDirections.length > 0 ? (
                reading.plausibleDirections.map((direction) => (
                  <div key={direction.id} className="border rounded-lg p-3">
                    ...
                    <p className="text-sm text-neutral-700">{direction.whyItFits}</p>
                  </div>
                ))
              ) : (
          ...
          <section className="border rounded-xl p-5 space-y-2">
            <h2 className="text-lg font-medium">Vectores de acción</h2>
            <div className="space-y-3">
              {reading.actionVectors.length > 0 ? (
                reading.actionVectors.map((vector) => (
                  <div key={vector.id} className="border rounded-lg p-3 space-y-2">
                    ...
                    <p className="text-sm text-neutral-700">{vector.description}</p>
          ...
          <section className="border rounded-xl p-5 space-y-2">
            <h2 className="text-lg font-medium">Camino mínimo</h2>
            <p className="text-sm text-neutral-700">{reading.summaryForUser.camino_minimo}</p>
          </section>
        </div>
